...
def read_map_and_oversample(map_in=None, mtz_in=None, mtz_labs=None, oversample_pixel=None):
    ...
    if oversample_pixel is not None:
        logger.write("--oversample_pixel= {} is requested.".format(oversample_pixel))
...
def map_peaks(args):
    ...
    logger.write("{} peaks detected".format(len(blobs)))
    ...
    logger.write("{} peaks after removing symmetry equivalents".format(len(blobs)))
    ...
    for b in blobs:
        ...
        if x is None:
            logger.write("too far from model: value={:.2f} volume= {} pos= {}".format(map_val, b.volume, bpos))
            continue
    ...
    if len(peaks) == 0:
        logger.write("No peaks found. Change parameter(s).")
        return
...
""")
    logger.write("\nRun:")
    logger.write("coot --script {}".format(coot_out))
# map_peaks()
