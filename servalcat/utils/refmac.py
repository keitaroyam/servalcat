...
def read_exte_line(l):
    ...
    # remove comment XXX these chars would not appear in instruction (atom names etc)?
    if "!" in l: l = l[:l.index("!")]
    if "#" in l: l = l[:l.index("#")]
    s = l.split()
    ret = dict(defaults={})
...
@profile"""
def read_external_restraints(lines, st, exte):
    ...
    extypes = dict(dist=gemmi.ExternalRestraints.Bond,
                   angl=gemmi.ExternalRestraints.Angle,
                   chir=gemmi.ExternalRestraints.Chirality,
                   tors=gemmi.ExternalRestraints.Torsion,
                   plan=gemmi.ExternalRestraints.Plane,
                   inte=gemmi.ExternalRestraints.Interval,
                   harm=gemmi.ExternalRestraints.Harmonic,
                   spec=gemmi.ExternalRestraints.Special,
                   stac=gemmi.ExternalRestraints.Stacking)
    exlists = dict(dist=exte.bonds, angl=exte.angles, tors=exte.torsions,
                   chir=exte.chirs, plan=exte.planes, inte=exte.intervals,
                   stac=exte.stackings, harm=exte.harmonics, spec=exte.specials)
    ...
    for l in lines:
        ...
        if r["rest_type"] == "dist":
            ...
            ex.sigma = min(max(ex.sigma, defs["sigma_min_loc"]), defs["sigma_max_loc"])
            ...
            if r["restr"].get("symm_in", defs["symall_block"]) or symm1:
                ...
                ex.set_image(im)
            print("dist=", ex.alpha, ex.type, ex.value, ex.sigma, ex.image_idx, ex.pbc_shift, ex.atoms)
        elif r["rest_type"] == "angl":
            print("angl=", ex.value, ex.sigma, ex.atoms)
        elif r["rest_type"] == "tors":
            ex.period = 1 # Refmac does not seem to read it from instruction
            print("tors=", ex.value, ex.sigma, ex.atoms)
    ...
    logger.writeln("External restraints")
    logger.writeln(" Number of distances         : {}".format(len(exte.bonds)))
    logger.writeln(" Number of angles            : {}".format(len(exte.angles)))
    logger.writeln(" Number of torsions          : {}".format(len(exte.torsions)))
    logger.writeln(" Number of planes            : {}".format(len(exte.planes)))
    logger.writeln(" Number of chirals           : {}".format(len(exte.chirs)))
    logger.writeln(" Number of intervals         : {}".format(len(exte.intervals)))
    logger.writeln(" Number of stackings         : {}".format(len(exte.stackings)))
    logger.writeln(" Number of harmonics         : {}".format(len(exte.harmonics)))
    logger.writeln(" Number of special positions : {}".format(len(exte.specials)))
    return exte
