...
If map has been symmetrised with a point group symmetry, asymmetric unit model should be given together with `--pg` to specify a point group symbol.
...
### Map trimming
The maps from single particle analysis often have very large size due to unnccesary region outside the molecule. You can save disk space by trimming the unnccesary region.
```
...
## Reference
Yamashita, K., Palmer, C. M., Burnley, T., Murshudov, G. N. (2021) "Cryo-EM single particle structure refinement and map calculation using Servalcat" *bioRxiv* submitted
